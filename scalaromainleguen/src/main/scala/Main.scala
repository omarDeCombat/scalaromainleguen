...
object Main extends JFXApp3 {
  ...
  private def updateDirection(directionState: ObjectProperty[Direction], key: KeyEvent): Unit = key.getCode match {
    case KeyCode.UP    => directionState.update(Direction.UP)
    case KeyCode.DOWN  => directionState.update(Direction.DOWN)
    case KeyCode.LEFT  => directionState.update(Direction.LEFT)
    case KeyCode.RIGHT => directionState.update(Direction.RIGHT)
    case _             => ()
  }
}
