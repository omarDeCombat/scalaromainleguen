...
final case class WatorState(tunas: Vector[Tuna], sharks: Vector[Shark]) {
  ...
  def move(windowSize: Int, cellSize: Int, sharkBreed: Int, tunaBreed: Int, sharkEnergie: Int, maxTunas: Int, maxShark: Int): WatorState = {
    val maxCells = windowSize / cellSize

    val tunaSet = tunas.filterNot(_.dead).map(tuna => (tuna.x, tuna.y) -> tuna.id).toSet
    ...
    val movedSharks = sharks.map { shark =>
      ...
      if (idTuna == -1) {
        val (newX, newY) = shark.direction match {
          case UP        => (shark.x, (shark.y - 1).max(0))
          case UPRIGHT   => ((shark.x + 1).min(maxCells - 1), (shark.y - 1).max(0))
          case RIGHT     => ((shark.x + 1).min(maxCells - 1), shark.y)
          case DOWNRIGHT => ((shark.x + 1).min(maxCells - 1), (shark.y + 1).min(maxCells - 1))
          case DOWN      => (shark.x, (shark.y + 1).min(maxCells - 1))
          case DOWNLEFT  => ((shark.x - 1).max(0), (shark.y + 1).min(maxCells - 1))
          case LEFT      => ((shark.x - 1).max(0), shark.y)
          case UPLEFT    => ((shark.x - 1).max(0), (shark.y - 1).max(0))
        }
        shark.copy(x = newX, y = newY)
      } else {
        shark.copy(x = tunax, y = tunay, energy = sharkEnergie)
      }
    }

    val movedTunas = tunas.flatMap { tuna =>
      if (sharkSet.contains((tuna.x, tuna.y))) {
        Vector() // Le thon disparaît
      } else {
        val (newX, newY) = tuna.direction match {
          case UP        => (tuna.x, (tuna.y - 1).max(0))
          case UPRIGHT   => ((tuna.x + 1).min(maxCells - 1), (tuna.y - 1).max(0))
          case RIGHT     => ((tuna.x + 1).min(maxCells - 1), tuna.y)
          case DOWNRIGHT => ((tuna.x + 1).min(maxCells - 1), (tuna.y + 1).min(maxCells - 1))
          case DOWN      => (tuna.x, (tuna.y + 1).min(maxCells - 1))
          case DOWNLEFT  => ((tuna.x - 1).max(0), (tuna.y + 1).min(maxCells - 1))
          case LEFT      => ((tuna.x - 1).max(0), tuna.y)
          case UPLEFT    => ((tuna.x - 1).max(0), (tuna.y - 1).max(0))
        }
        Vector(tuna.copy(x = newX, y = newY))
      }
    }
    ...
    val newState = copy(tunas = movedTunas, sharks = movedSharks)
      ...
      .sharkEnergie()
      .tunaBaby(maxCells, maxTunas, tunaBreed)
      .sharksBaby(maxCells, maxShark, sharkBreed, sharkEnergie)
  ...
  def sharkEnergie(): WatorState = {
    val sharksNRJ = sharks.flatMap { shark =>
      if (shark.energy == 0) {
        ...
        Vector(shark.copy(energy = shark.energy - 1))
      }
    }
  ...
  def tunaBaby(maxCells: Int, maxEntities: Int, tunaBreed: Int): WatorState = {
    ...
    val tunasBaby = tunas.flatMap { tuna =>
      if (tuna.breed <= 0 && tunas.length < maxEntities) {
        ...
        Vector(tuna.copy(breed = tuna.breed - 1))
      }
    }
  ...
  def sharksBaby(maxCells: Int, maxEntities: Int, sharkBreed: Int, sharkEnergie: Int): WatorState = {
    ...
    val sharksBaby = sharks.flatMap { shark =>
      if (shark.breed <= 0 && sharks.length < maxEntities) {
        ...
        Vector(shark.copy(breed = shark.breed - 1))
      }
    }
